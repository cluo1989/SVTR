...
class PatchEmbed(nn.Module):
    def __init__(self, 
        ...
        num_patches = (image_size[0] // 2**sub_num) * (image_size[1] // 2**sub_num)
        ...
        if mode == 'pope':
            if sub_num == 2:
                self.proj = nn.Sequential(
                    ConvBNLayer(in_channel=in_channel, 
                                out_channel=embed_dim//2, 
                                kernel_size=3, 
                                stride=2, 
                                padding=1, 
                                bias=False),
                    ConvBNLayer(in_channel=embed_dim//2,
            ...
            if sub_num == 3:
                self.proj = nn.Sequential(
                    ConvBNLayer(in_channel=in_channel,
                                out_channel=embed_dim//4,
                                kernel_size=3,
                                stride=2,
                                padding=1,
                                bias=False),
                    ConvBNLayer(in_channel=embed_dim//4,
                                out_channel=embed_dim//2,
                                kernel_size=3,
                                stride=2,
                                padding=1,
    ...
    def forward(self, inputs):
        ...
        # 输入 shape 校验，image_size, 32x100
        assert H == self.image_size[0] and W == self.image_size[1], \
            f"Input size {H}*{W} doesn't match model ({self.image_size[0]}*{self.image_size[1]})."
...
class SubSample(nn.Module):
    def __init__(self, 
        ...
        self.norm = eval(sub_norm)(out_channel)
        if act is not None:
            self.act = act()
        else:
            self.act = None
...
class SVTRNet(nn.Module):
    def __init__(self, 
        ...
        **kwargs
        ...
        self.HW = [image_size[0] // (2**sub_num), image_size[1] // (2**sub_num)]
        ...
        if use_lenhead:
            self.len_conv = nn.Linear(embed_dim[2], self.out_channel)
            self.hard_swish_len = nn.Hardswish()
    ...
    def forward_features(self, x):
        ...
        if self.patch_merging is not None:
            x = self.sub_sample1(x.permute([0,2,1])\
                .reshape([-1, self.embed_dim[0], self.HW[0], self.HW[1]]))
            
        for blk in self.blocks2:
            x = blk(x)

        if self.patch_merging is not None:
            x = self.sub_sample2(x.permute([0,2,1])\
                .reshape([-1, self.embed_dim[1], self.HW[0]//2, self.HW[1]]))
    ...
    def forward(self, x):
        ...
        if self.use_lenhead:
            ...
            len_x = self.dropout_len(self.hard_swish_len(len_x))
        ...
        if self.last_stage:
            ...
            x = self.avg_pool(x.permute([0, 2, 1]).reshape(
...
if __name__ == "__main__":
    # import numpy as np
    # model = PatchEmbed()
    # inputs = torch.randn(3,3,32,100) # NCHW
    # res = model(inputs)
    # print('--- res ---')
    # print(res)
